...
# Logo 	******************************************************************
CopyrightLogo="
                DS Video JavBus 搜刮器补丁（翻墙版本） $vVersion  QQ 群:160128165                                             
...
function install()
{
	...
	wget --no-check-certificate https://gitee.com/challengerV/dsm_javdb_patch/raw/master/dsm_javbus_fq_patch.tar -O dsm_javbus_fq_patch.tar;
	tar -xvf dsm_javbus_fq_patch.tar

	cp -rfa ./dsm_javbus_fq_patch/syno_themoviedb /var/packages/VideoStation/target/plugins/;
	cp -rfa ./dsm_javbus_fq_patch/syno_synovideodb /var/packages/VideoStation/target/plugins/;
	...
	cd -
	rm -rf dsm_javbus_fq_patch.sh
	echo '==========================================================================';
	echo "恭喜, DS Video JavBus 搜刮器补丁（翻墙版本） $vVersion 安装完成！";
	echo '==========================================================================';
}
...
{	
	...
	echo 'DS Video JavBus 搜刮器补丁（翻墙版本） 卸载完成！ QQ 群:160128165';
	echo '==========================================================================';
}
